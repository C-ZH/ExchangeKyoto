...
public class deletePost {
	...
	public void deleteRow(int post_id) {
		...
		try {
			...
			try {
				...
				String adj = "UPDATE post SET post_id = post_id - 1 WHERE post_id > " + post_id + ";";
				stmt_del.executeUpdate(sql); // remember to use "Update"
				stmt_adjust.executeUpdate(adj);
				
				// remove feedbacks in the same post
				sql = "SELECT * FROM re_post WHERE post_id=" + "'" + post_id + "';";
				...
				adj = "UPDATE re_post SET post_id = post_id - 1 WHERE post_id > " + post_id + ";";
				stmt_adjust.executeUpdate(adj);
	...
	public void deleteReRow(int re_post_id) {
		...
		try {
			...
			Statement stmt_adjust = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
			try {
				String sql = "DELETE FROM re_post WHERE re_post_id=" + "'" + re_post_id + "';";
				// after removing, change id;
				String adj = "UPDATE re_post SET re_post_id = re_post_id - 1 WHERE re_post_id > " + re_post_id + ";";
